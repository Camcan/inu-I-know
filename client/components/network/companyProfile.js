...
class CompanyProfile extends Component {
  ...
  render() {
    const co =
      this.props.companyList.filter((c) => {
        return c._id == this.props.selectedCompany;
      })[0] || {};
    const children = co.children
      ? this.props.companyList.filter((c) => {
        return co.children.includes(c._id);
      })
      : null;
    let parents = this.props.companyRels
      .filter((rel) => {
    ...
    return (
      <div className={[Styles.container, "box"].join(" ")}>
        <div className={Styles.top}>
          {co.logoUrl ? (
            <img className={Styles.logo} src={this.props.apiUrl + co.logoUrl} />
          ) : null}
